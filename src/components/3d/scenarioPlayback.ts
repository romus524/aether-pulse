...
export const findEventTime = (patient: PatientRecord, seekEvent?: string): number | null => {
  if (!seekEvent) return null;
  const needle = seekEvent.toLowerCase();
  const match = getScenarioForPatient(patient).keyframes.find((keyframe) => {
    return (
      keyframe.label.toLowerCase().includes(needle) ||
      keyframe.description.toLowerCase().includes(needle) ||
      keyframe.type.toLowerCase().includes(needle)
    );
  });
