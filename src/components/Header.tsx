...
export const Header: React.FC<HeaderProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const placeMenu = () => {
      ...
      setRoleMenuPos({ top: rect.bottom + 8, left });
    };
  ...
  useEffect(() => {
    const isInside = (root: HTMLElement | null, target: EventTarget | null) =>
      !!(root && target instanceof Node && root.contains(target));

    const handlePointer = (event: MouseEvent) => {
      const target = event.target;
      if (!isInside(dropdownRef.current, target)) {
        setSamplingDropdownOpen(false);
      }
      if (!isInside(roleDropdownRef.current, target) && !isInside(roleMenuRef.current, target)) {
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative shrink-0" ref={roleDropdownRef}>
            <button
              ...
              className={`px-2.5 py-1.5 rounded-xl border text-slate-200 flex items-center gap-1.5 text-xs font-sora font-semibold cursor-pointer shadow-sm group outline-none focus-visible:border-purple-400/60 ${
                roleDropdownOpen
                  ? 'bg-purple-950/80 border-purple-400/40'
                  : 'bg-slate-950/60 border-white/10 hover:border-purple-400/40'
              }`}
                  ...
                  {(Object.keys(ROLE_LABELS) as OperatorRole[]).map((role) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full px-2.5 py-2 text-left rounded-lg flex items-center justify-between cursor-pointer ${
                          selected
                            ? 'bg-purple-950/80 text-white border border-purple-500/40'
