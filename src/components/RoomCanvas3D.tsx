...
export const RoomCanvas3D: React.FC<RoomCanvasProps> = ({
  ...
  return (
    <div className="relative flex h-[540px] w-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#030712] shadow-2xl glass-panel glass-specular">
        ...
        <div className="pointer-events-auto flex flex-wrap items-center gap-1.5">
          <div className="flex items-center gap-1 rounded-full border border-white/10 p-1 shadow-lg glass-panel backdrop-blur-xl">
            ...
            <button
              ...
              className={`cursor-pointer rounded-full p-1.5 text-[10px] transition-all ${
                showDebugSkeleton ? 'border border-amber-500/40 bg-amber-500/20 text-amber-300' : 'text-slate-400 hover:text-white'
              }`}
            >
      ...
      <div className="h-full w-full bg-gradient-to-b from-[#020617] via-[#050814] to-[#020617]">
        ...
        >
          ...
          >
            ...
            {quality !== 'performance' && <Environment preset="apartment" environmentIntensity={0.28} />}
      ...
      <div className="pointer-events-none absolute right-3 bottom-[4.6rem] z-10 hidden items-center gap-1 rounded-full border border-white/10 px-2.5 py-1 font-tech text-[10px] text-slate-400 glass-panel backdrop-blur-xl sm:flex">
        <span>QUALITY:</span>
        {(['high', 'balanced', 'performance'] as const).map((q) => (
          <button
            key={q}
            onClick={() => setQuality(q)}
            className={`pointer-events-auto cursor-pointer rounded px-2 py-0.5 font-bold uppercase transition-all ${
              quality === q ? 'border border-cyan-500/30 bg-cyan-500/20 text-cyan-300' : 'hover:text-white'
            }`}
          >
            {q}
          </button>
